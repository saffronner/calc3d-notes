...
\newpage
\addsection{Chapter 6: Vector Calc}
\begin{multicols}{2}
  \col{
    ...
    \begin{itemize}
      ...
      \end{itemize}
      \item potential functions are unique to a constant (because they're found sorta with antiderivs??). ie, if $\nabla f = \nabla g = \vec{F}$, then $f = g + C$.
      \item to prove a field is not conservative:
      \begin{itemize}
        \item if $\vec{F}(x,y) = \left<P(x,y), Q(x,y)\right>$ \\ $\Rightarrow \frac{\partial P}{\partial y} = \frac{\partial Q}{\partial x}$.
        \item if $\vec{F} = \left<P, Q, R\right>$\\ $\Rightarrow \frac{\partial P}{\partial y} = \frac{\partial Q}{\partial x} \land \frac{\partial Q}{\partial z} = \frac{\partial R}{\partial y} \land \frac{\partial R}{\partial x} = \frac{\partial P}{\partial z}$
        \item use the contrapos.
      \end{itemize}
      \item to prove it is
      \begin{itemize}
        \item use the converse of above, only if $\vec{F}$ is on a open, simply connected region through which the converse holds
      \end{itemize}
    \end{itemize}
    Line Integrals
    \begin{itemize}
      \item scalar line integral (along curve in space)
      \begin{itemize}
      ...
      \end{itemize}
      \item vector line integral (along oriented curve)
      \item find work to move object along curve in a vec field
      \item flux and circulation of a vec field
    \end{itemize}
  }
\end{multicols}
